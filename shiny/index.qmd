...
:::{.callout-note}
Last updated to include games through: `September 17, 2022`.
:::
...
# load data
fgs_clean <- read_csv('fgs_09192022.csv')
...
fg_list <- list(by_kicker, fgs_clutch, xp_player) %>%
            reduce(inner_join, by = 'kicker_player_name') %>%
...
fgs_team <- fgs_clean %>%
            ...
            summarise(`Avg_Yds` = median(yds_fg),
                      `Pts/A` = format(round((sum(fgm)*3) / sum(fga)), nsmall = 1),
                      RZ_FGs = sum(rz),
                      TO_FG = sum(miss),
                      Avg_EPA = median(EPA),
                      FGA = sum(fga)) %>%
...
team_final <- team_main %>%
         rename(Team = logo, School = pos_team) %>%
